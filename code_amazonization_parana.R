...
summary_data <- read.csv("parana_study_case.csv", sep = ";")
...
dev_ppr<- gratia::derivatives(m_ppr_gam, select = "s(year)" )
print(dev_ppr,n=Inf)
...
results_summary <- data.frame(
  ...
  AICc_Linear = c(model_comp$AICc[1], model_comp_tir$AICc[1], ...),
  AICc_GAM    = c(model_comp$AICc[2], model_comp_tir$AICc[2], ...),
  ΔAICc = sapply(list(model_comp, model_comp_tir, ...),
...
metrics_models <- bind_rows(
  ...
  pivot_wider(names_from = Model, values_from = c(R2, R2_Nagelkerke)) %>%
  select(where(~ !all(is.na(.))))

results_summary <- results_summary %>%
  left_join(metrics_models, by = "Reservoir") %>%
...
AIC(tir_prop_linear,tir_prop_gam)
...
model_comp_ppr_prop <- compare_performance(
  ...
  metrics = c("AICc", "R2"))
model_comp_ppr_prop
AICcmodavg::AICc(ppr_prop_gam)
AICcmodavg::AICc(ppr_prop_linear)
